...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        query_states = self.q_norm(self._shape(self.q_proj(hidden_states), seq_len, bsz))
        key_states = self.k_norm(self._shape(self.k_proj(hidden_states), seq_len, bsz))
        ...
        # TODO: 根据第 6 步完成注意力计算
        output = F.softmax(
            (torch.matmul(query_states, key_states.transpose(-2, -1)) + attention_mask) / math.sqrt(self.head_dim),
            dim=-1,
        )
        output = torch.matmul(output, value_states)

        # TODO: 根据第 7 步完成输出投影
        output = self.o_proj(output.transpose(1, 2).contiguous().view(bsz, seq_len, -1))
